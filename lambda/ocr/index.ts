...
export const handler = async (event: S3Event): Promise<void> => {
  for (const record of event.Records) {
    ...
    // Expected key format: receipts/{userId}/{expenseId}.jpg
    // e.g. "receipts/abc-123-userId/def-456-expenseId.jpg"
    ...
    const expenseId = parts[2].replace(/\.[^/.]+$/, ""); // strip extension
    ...
    try {
      ...
      // SummaryFields: TOTAL, INVOICE_RECEIPT_DATE, VENDOR_NAME, etc.
      for (const doc of textractResult.ExpenseDocuments ?? []) {
        ...
        // LineItemGroups: individual receipt line items
      ...
      // ── Write OCR results back to the expense record ───────────────────────
      await ddb.send(
        new UpdateCommand({
          TableName:        TABLE,
      ...
      // Mark the expense as failed so the mobile app can show a retry option
      await ddb.send(
        new UpdateCommand({
          TableName:        TABLE,
          Key:              { userId, expenseId },
